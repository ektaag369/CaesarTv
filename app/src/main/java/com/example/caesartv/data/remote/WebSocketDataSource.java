        ...
        try {
            ...
            socket.on("registered_success", args -> {
                ...
                try {
                    ...
                    Log.d(TAG, "Fetching media for deviceId: " + deviceId);
                    fetchMediaFromApi(deviceId, listener, onError);
                } catch (Exception e) {
            ...
            socket.on("unblocked_device", args -> {
                ...
                fetchMediaFromApi(getDeviceId(), listener, onError);
            });
...
//            });
    ...
    private void fetchMediaFromApi(String deviceId, OnMediaFetchedListener listener, Runnable onError) {
        String url = API_BASE_URL + deviceId + "?page=1&limit=10";
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                retryApiCall(deviceId, listener, onError);
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    List<MediaItem> mediaList = parseMediaData(json);
                    if (!mediaList.isEmpty()) {
                        listener.onMediaFetched(mediaList);
                        Log.d(TAG, "Fetched " + mediaList.size() + " media items from API: " + mediaList);
                    } else {
                        Log.w(TAG, "No active media in API response");
                        onError.run();
                    }
                } catch (Exception e) {
    ...
    private void retryApiCall(String deviceId, OnMediaFetchedListener listener, Runnable onError) {
        if (retryCount < MAX_RETRIES && isNetworkAvailable()) {
            retryCount++;
            Log.d(TAG, "Retrying API call, attempt " + retryCount + "/" + MAX_RETRIES);
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                fetchMediaFromApi(deviceId, listener, onError);
            }, 2000 * retryCount); // Exponential backoff: 2s, 4s, 6s
        } else {
            Log.w(TAG, "Max API retries reached or no network, triggering onError");
            onError.run();
        }
    }
    ...
    private List<MediaItem> parseMediaData(JSONObject data) {
        ...
        try {
            JSONObject mediaData = data.optJSONObject("data");
            if (mediaData == null) {
                Log.w(TAG, "No 'data' field in response");
                return mediaList;
            }

            JSONArray mediaAllData = mediaData.optJSONArray("mediaAllData");
            if (mediaAllData == null) {
                Log.w(TAG, "No 'mediaAllData' field in response");
                return mediaList;
            }
    ...
    private void retryConnection(OnMediaFetchedListener listener, Runnable onBlocked, Runnable onError) {
        if (retryCount < MAX_RETRIES && isNetworkAvailable()) {
            retryCount++;
            Log.d(TAG, "Retrying WebSocket connection, attempt " + retryCount + "/" + MAX_RETRIES);
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
